...
```{r split-and-run_df, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
library(dplyr)
...
```{r umap-visualization, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE}
load(paste0(sobj_dir,"02_SI_sobj_df_out_list.Robj"))
     
src_list <- lapply(names(sobj_out_list), function(sample_name) {
  
  # Create a random suffix for the chunk label
  rand_suffix <- paste0(sample(letters, 3, replace=TRUE), collapse="")
  
  # Retrieve the processed object
  sobj_sub <- sobj_out_list[[sample_name]]
  
  # Panel tabset start
  panel_start <- ":::: {.panel-tabset}"
  
  # Tab heading for the sample
  tab_heading <- paste0("### Sample: ", sample_name)
  
  # Sub-heading for Doublet UMAP
  sub_heading <- "#### UMAP: Doublet classification"
  
  # Look for the doublet classification column
  DF_col <- grep("DF.classifications", colnames(sobj_sub@meta.data), value = TRUE)
  
  # Check if the classification column exists
  if (length(DF_col) > 0) {
    df_class_col <- DF_col[length(DF_col)]  # the newest classification column
    
    # Generate the plot code
    plot_code <- c(
      paste0("```{r doublet-umap-", sample_name, "-", rand_suffix, 
             ", fig.height=5, fig.width=5, echo=FALSE}"),
      paste0("sobj_sub <- sobj_out_list[['", sample_name, "']]"),
      paste0("DimPlot(sobj_sub, reduction='umap', group.by='", df_class_col, "') +"),
      paste0("  ggtitle('Doublet classification (", sample_name, "') + NoAxes()"),
      "```"
    )
    
    # Combine the components for this sample
    return(c(panel_start, tab_heading, sub_heading, plot_code, "::::"))
  } else {
    # If no doublet classification is found, indicate it
    return(c(panel_start, tab_heading, sub_heading, "No doublet classification found.\n", "::::"))
  }
})

# Combine all the chunks and insert them into the document
out <- knit_child(text = unlist(src_list), options = list(cache=FALSE))

# We'll assume `sobj_out_list` is already defined in your environment.

for (sample_name in names(sobj_out_list)) {
  
  # Retrieve the processed object
  sobj_sub <- sobj_out_list[[sample_name]]
  
  # Define panel/tabset markers
  panel_start <- ":::: {.panel-tabset}"
  panel_end   <- "::::"
  
  # Tab heading and sub-heading
  tab_heading <- paste0("### Sample: ", sample_name)
  sub_heading <- "#### UMAP: Doublet classification"
  ...
  if (length(DF_col) > 0) {
    df_class_col <- DF_col[length(DF_col)]  # newest classification column
    
    # Instead of creating an R chunk, we just print the DimPlot call as text
    cat(
      paste0(
        "DimPlot(sobj_sub, reduction='umap', group.by='", 
        df_class_col, 
        "') + ggtitle('Doublet classification (", 
        sample_name, 
        ")') + NoAxes()"
      ),
      sep = "\n"
...
```{r add_to_combined, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE}
# 1) Create a list of data frames, one per sample
df_list <- list()

for (sample_name in names(sobj_out_list)) {
  sobj_sub <- sobj_out_list[[sample_name]]
  
  # Identify the DF classification column(s) in this subset
  df_cols <- grep("^DF\\.classifications", colnames(sobj_sub@meta.data), value = TRUE)
  if (length(df_cols) == 0) {
    message("No DF.classifications column found for sample: ", sample_name)
    next
  }
  
  # If multiple DF columns, pick the last (newest) one
  df_col <- df_cols[length(df_cols)]
  
  # Extract cell IDs and classification vector
  sub_df <- data.frame(
    cell_id        = rownames(sobj_sub@meta.data),
    classification = sobj_sub@meta.data[[df_col]],
    sample         = sample_name,
    stringsAsFactors = FALSE
  )
  
  # Store it in the list
  df_list[[sample_name]] <- sub_df
}

# 2) Combine all sample data frames into one
df_combined <- do.call(rbind, df_list)
# Check structure
head(df_combined)
#   cell_id             classification  sample
# 1 AAACCTGAG...       Singlet         sample1
# 2 AAACGGGAG...       Doublet         sample1
# ... etc.

# 3) Create a data frame from the combined Seurat object’s rownames
#    so we can safely left_join on cell_id
df_filtered <- data.frame(
  cell_id = rownames(filtered_sobj@meta.data),
  stringsAsFactors = FALSE
)

# 4) Left-join df_combined onto df_filtered by cell_id
df_joined <- df_filtered %>%
  left_join(df_combined, by = "cell_id")

# This should yield a data frame with columns: 
#   cell_id, classification, sample
# For each cell in filtered_sobj, classification or sample might be NA 
# if it doesn’t match any from the subset objects.

# 5) Add that classification to filtered_sobj@meta.data
filtered_sobj@meta.data$DF_by_sample <- df_joined$classification
save(filtered_sobj, file = paste0(sobj_dir,tag,"_sobj_df.Robj"))
```

```{r visualize, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE}
load(paste0(sobj_dir,tag,"_sobj_df.Robj"))
p_doublets <- DimPlot(filtered_sobj, group.by = "DF_by_sample") + NoAxes()
  
counts_df <- filtered_sobj@meta.data %>%
  group_by(seurat_clusters, DF_by_sample) %>%
  summarise(Count = n()) %>%
  ungroup() %>%
...
p2 <- ggplot(
  counts_df, aes( x = factor(seurat_clusters, levels = sort(unique(seurat_clusters))),
    y = Proportion, fill = DF_by_sample)) +
  geom_bar(stat = "identity", position = "stack", width = 0.5) +
  labs( title = "", x = "Cluster", y = "", fill = "Classification") +
  theme_minimal() +
  theme(axis.text.x = element_text(hjust = 1))

p3 <- DimPlot(filtered_sobj, label = TRUE) + NoAxes() + NoLegend()
...
```{r 02_SI_12_post_df_filtering_qc_matrics, echo=TRUE, eval=TRUE}
...
```{r 02_SI_13_post_filtering_pca, echo=TRUE, eval=TRUE}
...
```{r 02_SI_14_post_filtering_umap, echo=TRUE, eval=TRUE}
...
```{r 02_SI_15_post_filtering_tsne, echo=TRUE, eval=TRUE}
...
```{r 02_SI_16_gene_exp_freq, echo=TRUE, eval=TRUE}
...
```{r 02_SI_18_pca_outliers_counts_features, echo=TRUE, eval=TRUE}
...
```{r doublets_main, echo=TRUE, eval=FALSE, cache=FALSE}
...
```{r 02_SI_main_3_validation, echo=TRUE, eval=TRUE, cache=FALSE}
library(DT)
...
```{r 02_SI_main_4_validation, echo=TRUE, eval=TRUE, cache=FALSE}
...
```{r 02_SI_main_5_validation, echo=TRUE, eval=TRUE, cache=FALSE}
...
```{r 02_SI_main_6_validation, echo=TRUE, eval=TRUE, cache=FALSE}
...
```{r 02_SI_main_7_validation, echo=TRUE, eval=TRUE, cache=FALSE}
